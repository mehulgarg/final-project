...
public:
	...
	void recover(const int expectedAgentCount,
	...
	void updateInverseOffer(const SlaveID &slaveId,
...
public:
	...
	virtual void initialize(const mesos::allocator::Options &options,
			const lambda::function<
					void(const FrameworkID&,
							const hashmap<std::string,
									hashmap<SlaveID, Resources>>&)> &offerCallback,
			const lambda::function<
					void(const FrameworkID&,
							const hashmap<SlaveID, UnavailableResources>&)> &inverseOfferCallback) = 0;

	virtual void recover(const int expectedAgentCount,
	...
	virtual void updateFramework(const FrameworkID &frameworkId,
			const FrameworkInfo &frameworkInfo,
			const std::set<std::string> &suppressedRoles) = 0;

	virtual void addSlave(const SlaveID &slaveId, const SlaveInfo &slaveInfo,
			const std::vector<SlaveInfo::Capability> &capabilities,
			const Option<Unavailability> &unavailability,
	...
	virtual void addResourceProvider(const SlaveID &slave,
			const Resources &total,
			const hashmap<FrameworkID, Resources> &used) = 0;
	...
	virtual void requestResources(const FrameworkID &frameworkId,
			const std::vector<Request> &requests) = 0;

	virtual void updateAllocation(const FrameworkID &frameworkId,
			const SlaveID &slaveId, const Resources &offeredResources,
			const std::vector<ResourceConversion> &conversions) = 0;

	virtual process::Future<Nothing> updateAvailable(const SlaveID &slaveId,
			const std::vector<Offer::Operation> &operations) = 0;

	virtual void updateUnavailability(const SlaveID &slaveId,
			const Option<Unavailability> &unavailability) = 0;

	virtual void updateInverseOffer(const SlaveID &slaveId,
			const FrameworkID &frameworkId,
			const Option<UnavailableResources> &unavailableResources,
			const Option<mesos::allocator::InverseOfferStatus> &status,
	...
	virtual void recoverResources(const FrameworkID &frameworkId,
			const SlaveID &slaveId, const Resources &resources,
			const Option<Filters> &filters) = 0;

	virtual void suppressOffers(const FrameworkID &frameworkId,
			const std::set<std::string> &roles) = 0;

	virtual void reviveOffers(const FrameworkID &frameworkId,
			const std::set<std::string> &roles) = 0;
...
template<typename AllocatorProcess>
Try<mesos::allocator::Allocator*> MesosAllocator<AllocatorProcess>::create() {
...
inline void MesosAllocator<AllocatorProcess>::initialize(
		...
		const lambda::function<
				void(const FrameworkID&,
						const hashmap<SlaveID, UnavailableResources>&)> &inverseOfferCallback) {
	process::dispatch(process, &MesosAllocatorProcess::initialize, options,
			offerCallback, inverseOfferCallback);
}
...
inline void MesosAllocator<AllocatorProcess>::recover(
	...
	process::dispatch(process, &MesosAllocatorProcess::recover,
			expectedAgentCount, quotas);
}
...
inline void MesosAllocator<AllocatorProcess>::addFramework(
	...
	process::dispatch(process, &MesosAllocatorProcess::addFramework,
			frameworkId, frameworkInfo, used, active, suppressedRoles);
}

template<typename AllocatorProcess>
inline void MesosAllocator<AllocatorProcess>::removeFramework(
		const FrameworkID &frameworkId) {
	process::dispatch(process, &MesosAllocatorProcess::removeFramework,
			frameworkId);
}
...
inline void MesosAllocator<AllocatorProcess>::activateFramework(
		const FrameworkID &frameworkId) {
	process::dispatch(process, &MesosAllocatorProcess::activateFramework,
			frameworkId);
}
...
inline void MesosAllocator<AllocatorProcess>::deactivateFramework(
		const FrameworkID &frameworkId) {
	process::dispatch(process, &MesosAllocatorProcess::deactivateFramework,
			frameworkId);
}
...
inline void MesosAllocator<AllocatorProcess>::updateFramework(
		const FrameworkID &frameworkId, const FrameworkInfo &frameworkInfo,
		const std::set<std::string> &suppressedRoles) {
	process::dispatch(process, &MesosAllocatorProcess::updateFramework,
			frameworkId, frameworkInfo, suppressedRoles);
}

template<typename AllocatorProcess>
inline void MesosAllocator<AllocatorProcess>::addSlave(const SlaveID &slaveId,
		const SlaveInfo &slaveInfo,
		const std::vector<SlaveInfo::Capability> &capabilities,
		const Option<Unavailability> &unavailability, const Resources &total,
		const hashmap<FrameworkID, Resources> &used) {
	process::dispatch(process, &MesosAllocatorProcess::addSlave, slaveId,
			slaveInfo, capabilities, unavailability, total, used);
}

template<typename AllocatorProcess>
inline void MesosAllocator<AllocatorProcess>::removeSlave(
		const SlaveID &slaveId) {
	process::dispatch(process, &MesosAllocatorProcess::removeSlave, slaveId);
}
...
inline void MesosAllocator<AllocatorProcess>::updateSlave(
		...
		const Option<std::vector<SlaveInfo::Capability>> &capabilities) {
	process::dispatch(process, &MesosAllocatorProcess::updateSlave, slaveId,
			slaveInfo, total, capabilities);
}

template<typename AllocatorProcess>
void MesosAllocator<AllocatorProcess>::addResourceProvider(const SlaveID &slave,
		const Resources &total, const hashmap<FrameworkID, Resources> &used) {
	process::dispatch(process, &MesosAllocatorProcess::addResourceProvider,
			slave, total, used);
}
...
inline void MesosAllocator<AllocatorProcess>::activateSlave(
		const SlaveID &slaveId) {
	process::dispatch(process, &MesosAllocatorProcess::activateSlave, slaveId);
}

template<typename AllocatorProcess>
inline void MesosAllocator<AllocatorProcess>::deactivateSlave(
		const SlaveID &slaveId) {
	process::dispatch(process, &MesosAllocatorProcess::deactivateSlave,
			slaveId);
}
...
inline void MesosAllocator<AllocatorProcess>::updateWhitelist(
	...
	process::dispatch(process, &MesosAllocatorProcess::updateWhitelist,
			whitelist);
}
...
inline void MesosAllocator<AllocatorProcess>::requestResources(
	...
	process::dispatch(process, &MesosAllocatorProcess::requestResources,
			frameworkId, requests);
}
...
inline void MesosAllocator<AllocatorProcess>::updateAllocation(
	...
	process::dispatch(process, &MesosAllocatorProcess::updateAllocation,
			frameworkId, slaveId, offeredResources, conversions);
}

template<typename AllocatorProcess>
inline process::Future<Nothing> MesosAllocator<AllocatorProcess>::updateAvailable(
		const SlaveID &slaveId,
		const std::vector<Offer::Operation> &operations) {
	return process::dispatch(process, &MesosAllocatorProcess::updateAvailable,
			slaveId, operations);
}

template<typename AllocatorProcess>
inline void MesosAllocator<AllocatorProcess>::updateUnavailability(
		const SlaveID &slaveId, const Option<Unavailability> &unavailability) {
	return process::dispatch(process,
			&MesosAllocatorProcess::updateUnavailability, slaveId,
			unavailability);
}
...
inline void MesosAllocator<AllocatorProcess>::updateInverseOffer(
	...
	return process::dispatch(process,
			&MesosAllocatorProcess::updateInverseOffer, slaveId, frameworkId,
			unavailableResources, status, filters);
}

template<typename AllocatorProcess>
inline process::Future<
		hashmap<SlaveID,
				hashmap<FrameworkID, mesos::allocator::InverseOfferStatus>>> MesosAllocator<
		AllocatorProcess>::getInverseOfferStatuses() {
	return process::dispatch(process,
...
inline void MesosAllocator<AllocatorProcess>::recoverResources(
	...
	process::dispatch(process, &MesosAllocatorProcess::recoverResources,
...
inline void MesosAllocator<AllocatorProcess>::suppressOffers(
	...
	process::dispatch(process, &MesosAllocatorProcess::suppressOffers,
			frameworkId, roles);
}
...
inline void MesosAllocator<AllocatorProcess>::reviveOffers(
		const FrameworkID &frameworkId, const std::set<std::string> &roles) {
	process::dispatch(process, &MesosAllocatorProcess::reviveOffers,
			frameworkId, roles);
}

template<typename AllocatorProcess>
inline void MesosAllocator<AllocatorProcess>::updateQuota(
		const std::string &role, const Quota &quota) {
	process::dispatch(process, &MesosAllocatorProcess::updateQuota, role,
			quota);
}
...
inline void MesosAllocator<AllocatorProcess>::updateWeights(
	...
	process::dispatch(process, &MesosAllocatorProcess::updateWeights,
			weightInfos);
}
